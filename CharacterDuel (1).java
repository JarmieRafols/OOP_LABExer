public class CharacterDuel implements Weapon, Armor {
    ...
    static Wizard wizard = new Wizard("JAJA",3,4,5);
    static Wizard swordsman = new Wizard("Pretzels",6,5,4);
    static Dice dice = new Dice();
    public static void main(String[] args) {
        for(int i=0; i<10; i++) {
            if(wizard.getCurrentMagic() <= 10) {
                swordsman.wound(wizard.castLightningBolt());
                System.out.println(wizard.getName() + " casts lightning bolt to " + swordsman.getName() + " for " + wizard.castLightningBolt());
